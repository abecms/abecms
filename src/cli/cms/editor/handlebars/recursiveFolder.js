...
export default function recursiveFolder(obj, index = 1, dataShow = '', links = null, visible = false, translate) {
  var classHidden = ''
  if(index > 1 && !visible) {
    classHidden = 'hidden'
  }
  ...
  var levelText = (translate && translate['level-' + index] != null) ? translate['level-' + index] : 'Level ' + index
  ...
  Array.prototype.forEach.call(obj, (o) => {
    ...
    if(o.folders != null && o.folders.length > 0) {
      sub += recursiveFolder(o.folders, index+1, name.replace(/\.| |\#/g, '_'), links, isVisible, translate)
    }
  })
