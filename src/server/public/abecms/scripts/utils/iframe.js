...
export function IframeNode(frameId, selector) {
  ...
  if (iframe) {
    ...
    if (result.length === 0) {
      let key
      if (selector[0] === '[') key = selector.slice(1, -1)
      else key = selector
          ...
          if (matchFromExec != null && matchFromExec[1] != null) {
            ...
            node.setHtml = function(val) {
              style.textContent = style.textContent.replace(
                matchFromExec[1] + matchFromExec[2] + matchFromExec[3],
                matchFromExec[1] + val + matchFromExec[3]
              )
            }
            result = []
            result.push(node)
  ...
  for (var cn = nd.childNodes, i = 0, mx = cn.length; i < mx; i++) {
    ...
    if (
      any
        ? it[prop]
        : it[prop] !== undefined &&
          (meth ? ''[meth] && String(it[prop])[meth](val) : it[prop] == val)
    ) {
      r[r.length] = it
    }
  }
