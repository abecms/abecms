...
export default class UserLogin {
  constructor(enforcer) {
    ...
    if (enforcer != singletonEnforcer)
      throw 'Cannot construct UserLogin singleton'
  }
  ...
  init() {
    ...
    this.isInit = true
    // const
    this._btnActions = [].slice.call(
      document.querySelectorAll('.form-wrapper .btns [data-workflow]')
    )
    this._formWrapper = document.querySelector('.abeform-wrapper')
    if (this._btnActions.length > 0) {
      this._btnHidden = document.querySelector(
        '.form-wrapper .btns [data-action="draft"]'
      )
      this._btnReject = document.querySelector(
        '.form-wrapper .btns [data-extra-btn="reject"]'
      )
      this._btnEdit = document.querySelector(
        '.form-wrapper .btns [data-extra-btn="edit"]'
      )

      this._inputs = [].slice.call(
        this._formWrapper.querySelectorAll(
          '.tab-pane:not([id=slug]) input.form-abe'
        )
      )
      this._inputs = this._inputs.concat(
        [].slice.call(
          this._formWrapper.querySelectorAll(
            '.tab-pane:not([id=slug]) textarea.form-abe'
          )
        )
      )
      this._inputsFile = [].slice.call(
        this._formWrapper.querySelectorAll(
          '.tab-pane:not([id=slug]) .upload-wrapper input[type="file"]'
        )
      )
      this._selects = [].slice.call(
        this._formWrapper.querySelectorAll('.tab-pane:not([id=slug]) select')
      )
      this._inputHasChanged = false
      this._checkInputChanged =
        typeof this._btnHidden !== 'undefined' && this._btnHidden !== null
          ? true
          : false
  ...
  _csrfToken() {
    ...
    ;(function(open) {
      XMLHttpRequest.prototype.open = function(
        ...
        var domain =
          url.indexOf('://') > -1 ? url.split('/')[2] : window.location.hostname
  ...
  _showHideBtn() {
    this._changeCurrentBtn(
      document.querySelector(
        '.form-wrapper .btns [data-action="' + json.abe_meta.status + '"]'
      )
    )
